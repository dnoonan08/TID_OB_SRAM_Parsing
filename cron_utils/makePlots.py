...
from June_2026_TID_utils import loadData, mark_TID_times, _xray_times
import numpy as np
...
def voltage_summary(d_summary, mark_TID_times, _COB_, plotTID=False, xlim=(None,None), ylim=(None,None)):
    ...
    cut=d.pp_passing_v!= 1.40
    ax.plot(x[cut],d.pp_passing_v[cut],label='PP BIST')
    ax.plot(x[cut],d.ob_passing_v[cut],label='OB BIST')
    ax.plot(x[cut],d.i2c_drop_v[cut],label='I2C dropout')
...
def temperaturePlot(d_tot,
    ...
    fig,ax = plt.subplots(1,1)
    for v in voltages:
        if not v in d_tot.index: continue
        d = d_tot.loc[v]
...
def makeSummaryPlots(_COB_):
    ...
    d_summary.sort_values('timestamps',inplace=True)
    if not os.path.exists(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}'):
        os.mkdir(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}')

    f = voltage_summary(d_summary, mark_TID_times, _COB_);
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/VoltageSummary_{_COB_}.pdf', bbox_inches='tight')
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/VoltageSummary_{_COB_}.png', bbox_inches='tight')

    f = voltage_summary(d_summary, mark_TID_times, _COB_, plotTID=True);
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/VoltageSummary_TID_{_COB_}.pdf', bbox_inches='tight')
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/VoltageSummary_TID_{_COB_}.png', bbox_inches='tight')

    f = voltage_summary(d_summary, mark_TID_times, _COB_, xlim=(-2,15), plotTID=True);
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/VoltageSummary_TID_Turnon_{_COB_}.pdf', bbox_inches='tight')
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/VoltageSummary_TID_Turnon_{_COB_}.png', bbox_inches='tight')

    f = temperaturePlot(d_tot, mark_TID_times, _COB_,xlim=(_startTime, None),bad_temps=[])
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/Temperature_{_COB_}.pdf', bbox_inches='tight')
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/Temperature_{_COB_}.png', bbox_inches='tight')

    f = currentPlot(d_tot, mark_TID_times, _COB_,xlim=(_startTime, None))
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/Current_{_COB_}.pdf', bbox_inches='tight')
    f.savefig(f'/eos/user/d/dnoonan/June_2026_TID_Data/plots/{_COB_}/Current_{_COB_}.png', bbox_inches='tight')
